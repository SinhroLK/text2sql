...
| Experiment runner/evaluator | Results | EVAL-001 structured results; EVAL-002 strict resource resolution, protected references, exact-ID summaries, frozen configs and generated tables |

`DATA-001` evidence is the pinned Spider2-Lite protocol, the database-disjoint split manifest, ADR-003 and `tests/test_dataset_protocol.py`. `DATA-003` evidence is the metadata manifest, ADR-004, checksum-gated loader, preparation CLI and `tests/test_spider2_loader.py`. `EVAL-001` evidence is ADR-005, `docs/evaluation.md`, the isolated executor/comparator and `tests/test_execution_evaluator.py`. `EVAL-002` evidence is ADR-006, `docs/spider2-evaluation-runner.md` and `tests/test_spider2_evaluation_runner.py`; it must not be described as complete until the real 31-example run succeeds. The methodology chapter must state that the score covers a custom 104-instance SQLite holdout, not the full 547-instance leaderboard setting.
